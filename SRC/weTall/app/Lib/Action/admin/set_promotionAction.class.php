...
class set_promotionAction extends backendAction {
	...
	public function _before_index() {
		...
		$big_menu = array(
				'title' => '添加促销活动',
				'iframe' => U('set_promotion/add',array('tokenTall'=>$tokenTall)),
				'id' => 'add',
				'width' => '520',
				'height' => '410',
		);
	...
	public function start(){
		...
		$status = $set_promotion->where($where)->find();
		if ($status["status"] == "2") {
			$message = "活动已经结束";
			...
			$data["status"] = 1;
			if ($set_promotion->where($where)->save($data)) {
				$message = "设置活动开始成功";;
			}
		}
		$this->ajaxReturn(1, L('operation_success'), $tags);
